...
use crate::utils::{calculate_complexity, extract_contract_name};
use anyhow::Result;
use std::fs;
...
impl Analyzer {
    ...
    pub async fn analyze_with_network(&self, network: &NetworkConfig) -> Result<AnalysisResults> {
        ...
        {
            self.estimate_resources(&mut results, network)?;
        }

        if self.enabled_checks.is_empty()
            || self.enabled_checks.contains(&"compatibility".to_string())
        {
        ...
        if self.enabled_checks.is_empty()
            || self.enabled_checks.contains(&"best-practices".to_string())
        {
    ...
    fn estimate_resources(
        ...
        let storage_deposit = storage_usage * network.storage_deposit_per_byte;
        ...
        for opcode in unsupported_opcodes {
            if source.contains(opcode) {
                results
                    ...
                    .push(format!("Unsupported opcode '{}' used in contract", opcode));
            }
        }
    ...
    fn check_security(&self, results: &mut AnalysisResults) -> Result<()> {
        ...
        if source.contains("call") && !source.contains("reentrancy") {
            results.security_vulnerabilities.push(
                "Potential reentrancy vulnerability: External calls without reentrancy guard"
                    .to_string(),
            );
        }
        ...
        if source.contains("send") && !source.contains("require") {
            results
                ...
                .push("Unchecked send operation: Missing require statement after send".to_string());
        }
    ...
    fn check_best_practices(&self, results: &mut AnalysisResults) -> Result<()> {
        ...
        if !source.contains("pragma solidity") {
            results
                .best_practices
                .push("Missing pragma directive: Should specify Solidity version".to_string());
        }
            ...
            && !source.contains("external")
        {
            results
                .best_practices
                .push("Missing function visibility modifier".to_string());
        }
